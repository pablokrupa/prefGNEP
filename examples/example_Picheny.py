"""
Script to solve the example from Section 4.1 of:
    Picheny, V., Binois, M., & Habbal, A. (2019). A Bayesian optimization approach to find Nash
    equilibria. Journal of Global Optimization, 73(1), 171-192.
...
beq = None
lb = np.array([-5.0, 0.0])  # Lower bounds
ub = np.array([10.0, 15.0])  # Upper bounds
...
# Initialize PrefGNEP
th_0, fc, th_min, th_max = gen_quad_models(sizes, full=False, with_linear=False)
...
if gen_dataset:
    ...
    with open("example_picheny_dataset.pkl", "wb") as temp_file:
        pickle.dump((ds.samples, ds.xA, ds.xB, ds.prefs), temp_file)
else:
    print("\n=== Loading initial dataset and feasibility check ===")
    with open("example_picheny_dataset.pkl", "rb") as temp_file:
...
plt.show()
if save_plots:
    fig_x_star.savefig('figs/fig_picheny_x_star.eps', format='eps', bbox_inches="tight")
...
plt.show()
if save_plots:
    fig_br_dev.savefig('figs/fig_picheny_br_dev.eps', format='eps', bbox_inches="tight")
...
plt.show()
if save_plots:
    fig_acc.savefig('figs/fig_picheny_acc.eps', format='eps', bbox_inches="tight")
